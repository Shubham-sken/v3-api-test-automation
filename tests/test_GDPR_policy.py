...
def call_GDPR_policy_endpoint(api_crm_base_url, orgid, auth_headers, additional_params=None):
    if orgid in [None, 'nan', '']:  # Check for None, 'nan', or empty string
        endpoint = "/v3/organizations//gdpr"
    else:
        endpoint = f"/v3/organizations/{orgid}/gdpr"
...
# Test case function
@pytest.mark.parametrize("index, row", pd.read_csv(r"TestData\GDPR.csv").iterrows())
def test_GDPR_policy_api(index, row, api_crm_base_url, auth_headers):
