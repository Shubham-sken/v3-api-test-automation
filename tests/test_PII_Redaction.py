...
import pandas as pd
import json
from utils.request_helper import get  # Assuming your request_helper file is used
...
def call_PII_Redaction(api_base_url, orgid, auth_headers, additional_params=None):
    if orgid in [None, 'nan', '']:  # Check for None, 'nan', or empty string
        endpoint = "/v3/organizations//admin/pii-redaction" 
    else:
        endpoint = f"/v3/organizations/{orgid}/admin/pii-redaction"
...
# Test case function
@pytest.mark.parametrize("index, row", pd.read_csv(r"TestData\PIIRedaction.csv").iterrows())
def test_PIIRedaction_api(index, row, api_base_url, auth_headers):
    # Fetch orgid from the test data
    orgid = row['orgid'] if pd.notna(row['orgid']) else None
    # Call the service API endpoint
    api_response = call_PII_Redaction(api_base_url, orgid, auth_headers)
