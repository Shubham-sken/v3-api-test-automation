...
def call_service_endpoint(api_base_url, orgid, auth_headers, additional_params=None):
    if orgid in [None, 'nan', '']:  # Check for None, 'nan', or empty string
        endpoint = "/v3/organizations//services" 
    else:
        endpoint = f"/v3/organizations/{orgid}/services"
...
# Test case function
@pytest.mark.parametrize("index, row", pd.read_csv(r"TestData\ServiceEnablement.csv").iterrows())
def test_service_api(index, row, api_base_url, auth_headers):
    # Fetch orgid from the test data
    orgid = row['orgid'] if pd.notna(row['orgid']) else None
    # Call the service API endpoint
    api_response = call_service_endpoint(api_base_url, orgid, auth_headers)
