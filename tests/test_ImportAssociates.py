...
def call_ImportAssociates_endpoint(api_base_url, orgid,crmid, auth_headers, additional_params=None):
    if orgid in [None, 'nan', '']:  # Check for None, 'nan', or empty string
        endpoint = "/v3/organizations//crms//associates" 
    else:
        endpoint = f"/v3/organizations/{orgid}/crms/{crmid}/associates"
...
# Test case function
@pytest.mark.parametrize("index, row", pd.read_csv(r"TestData\ImportAssociates.csv").iterrows())
def test_ImportAssociates_api(index, row, api_base_url, auth_headers):
    # Fetch orgid from the test data
    orgid = row['orgid'] if pd.notna(row['orgid']) else None
    crmid=row['crm']
    # Call the service API endpoint
    api_response = call_ImportAssociates_endpoint(api_base_url, orgid,crmid, auth_headers)
