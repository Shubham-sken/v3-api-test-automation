...
def call_ConnectCRM_endpoint(api_crm_base_url, orgid, auth_headers, additional_params=None):
    if orgid in [None, 'nan', '']:  # Check for None, 'nan', or empty string
        endpoint = "/v3/system/crms/organizations/" 
    else:
        endpoint = f"/v3/system/crms/organizations/{orgid}"
...
def assert_api_response(index, row, api_response):
    ...
    if expected_response is not None:
        ...
        print(f"Row {index} Debug Info:")
        print(f"  Expected Response: {expected_response}")
        print(f"  Actual Response: {normalized_api_response}")
        
        # Find differences between expected and actual response
        api_response_str = normalized_api_response.get('response', None)
        if api_response_str != expected_response:
            print(f"Row {index}: Mismatch Detected")
...
# Test case function
@pytest.mark.parametrize("index, row", pd.read_csv(r"TestData\ConnectCRM.csv").iterrows())
def test_ConnectCRM_api(index, row, api_crm_base_url, auth_headers):
    # Fetch orgid from the test data
    orgid = row['orgid'] if pd.notna(row['orgid']) else None
    # Call the service API endpoint
    api_response = call_ConnectCRM_endpoint(api_crm_base_url, orgid, auth_headers)
