...
def call_service_endpoint(api_base_url, orgid, auth_headers, additional_params=None):
    if orgid in [None, 'nan', '']:  # Check for None, 'nan', or empty string
        endpoint = "/v3/organizations//pipelines" 
    else:
        endpoint = f"/v3/organizations/{orgid}/pipelines"
...
# Test case function
@pytest.mark.parametrize("index, row", pd.read_csv(r"TestData\ImportPipeline.csv").iterrows())
