...
def call_DiscoverWinLoss_endpoint(api_base_url, orgid,crmid, auth_headers, additional_params=None):
    if orgid in [None, 'nan', '']:  # Check for None, 'nan', or empty string
        endpoint = "/v3/organizations//crms//win-loss-mapping"
    else:
        endpoint = f"/v3/organizations/{orgid}/crms/{crmid}/win-loss-mapping"
...
@pytest.mark.parametrize("index, row", pd.read_csv(r"TestData\DiscoverWinLoss.csv").iterrows())
def test_DiscoverWinLoss_api(index, row, api_base_url, auth_headers):
    orgid = row['orgid'] if pd.notna(row['orgid']) else None
    crmid=row['crm']
    api_response = call_DiscoverWinLoss_endpoint(api_base_url, orgid,crmid, auth_headers)
